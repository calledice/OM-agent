...
_u_arg.py实现了——_00.py,_01.py和_u.py的集成，通过输入参数可以选择三种功能
"""
from datetime import datetime
import json
import re
        ...
        "system_log": {
            ...
            "system": {
                "uptime": {
                    ...
                    "load_average": psutil.getloadavg()[0] / psutil.cpu_count()   # 标准化负载 
                }
            },
            "hardware": {
                "memory": {
...
def main(user_input,xxx,sub_task_id):
    model_name = "Qwen/Qwen2-0.5B-Instruct"
    tokenizer = AutoTokenizer.from_pretrained(model_name) 
    model = AutoModelForCausalLM.from_pretrained(model_name,  device_map="auto")
    # 定义诊断函数 
    def diagnostics(user_query: str,xxx:str,sub_task_id:int) -> str:
        """检查系统在指定日期的状况"""
        print(f"正在检查系统状况...")
        if sub_task_id !=2:
            # 1. 提取日期 
            log_name = extract_date_from_query(user_query)
            if not log_name:
    ...
    response = tokenizer.decode(outputs[0],  skip_special_tokens=True)# response, num_tokens, qps = run_llm_inference(prompt)

    # print(response[0]["generated_text"] )

    # 检查是否需要调用诊断功能 
    if "diagnostics" in response.lower()  or "需要" in response.lower()or "检查" in response.lower(): 
        # 直接调用诊断函数 
        func_response = diagnostics(user_input,xxx,sub_task_id)
...
if __name__ == "__main__": 
    ...
    parser = argparse.ArgumentParser(description="训练模型的参数配置")

    parser.add_argument("--sub-task-id", type=int, default=0, help="默认 0")
    parser.add_argument("--utc-time", type=int, default=0, help="默认 0")
    ...
    main(user_input,xxx,sub_task_id) 
